# 進捗
![alt](./ReleaseImage/alpha-5/ライセンス出力画面.png)
![alt](./ReleaseImage/alpha-5/素材追加画面.png)

1. 素材の追加画面を改めて実装しなおしました。
   1. ファイルの種類を新たに追加しました。
   2. 素材配布サイト名を直接入力しなくても、コンポボックスから選択できるようになりました。
   3. 実際の動作は、今後追加していく予定です。
2. ライセンス表示の出力画面を改めて実装しなおしました。
   1. ライセンス表示本文の前と後ろに文章を設定できるようになりました。
   2. 実際の動作は、今後追加していく予定です。

# このバージョンの素材ライセンスチェッカーをダウンロードするには?
## GoogleDriveからダウンロードする
以下のリンクにある**alpha-5.zip**というファイルをダウンロードしてください。
